import{u as s,s as l}from"./ui-C3ZGStvc.js";const g=(e,o=document)=>o.querySelector(e),f=g("#mesasContainer");g("#anio").textContent=new Date().getFullYear();const x=new URLSearchParams(window.location.search).get("token");if(!x)throw s.error("No se encontró el token de acceso."),new Error("Token faltante");s.loading("Cargando mesas...");let n=null,h=[],p=[],d=null;function _(){return(n.opciones_comun||0)+(n.opciones_celiacos||0)+(n.opciones_vegetarianos||0)+(n.opciones_veganos||0)}function w(){p=[];for(const e of h){const o=e.mesa_asientos||[];for(const a of o)a?.invitado_id===n.id&&p.push({mesa_id:e.id,posicion:a.posicion,id:a.id,cambios:a.cambios||0})}}function b(){const e=_(),o=p.length,a=e-o;let r=`
  ...
  `;g("#estadoAsientos")?.remove();const m=document.createElement("div");m.id="estadoAsientos",m.innerHTML=r,f.before(m)}async function v(){try{const{data:e,error:o}=await l.from("invitados").select("*").eq("mesa_token",x).maybeSingle();if(o||!e)throw new Error("Token inválido");n=e;const{data:a,error:r}=await l.from("mesas").select(`
        id,
        numero,
        capacidad,
        mesa_asientos (
